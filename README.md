...
## 15. Possible Jury Questions & Answers

> Read these out loud once before presenting. Confidence comes from having answered the question already in your head.

**1. How do you identify the vehicle make and model?**
We use image classification to predict color, make, and model from the photo, and show a confidence score. We're honest that precise make/model/year recognition is one of the harder problems, so we treat it as a *correctable best guess*: the user can confirm or fix it, and in many cases they can simply tell us the model — they know their own car. The vehicle identity mainly anchors the price search, so a user correction keeps the estimate accurate even when the model is unsure.

**2. How do you detect which part is damaged?**
Our object-detection model first locates the damaged regions and draws bounding boxes. Each region is then mapped to a known car part — bumper, door, hood, fender, headlight, and so on. In the MVP we use detection (boxes); a future upgrade is segmentation, which traces the exact outline of each part for cleaner boundaries.

**3. How accurate is the cost estimation?**
It's an *indicative range*, not an exact figure, and we're deliberate about that framing. The estimate is only as good as the online listings available, and the real cost depends on the specific shop, the exact part variant, and any hidden damage. That's why we always show a minimum and a maximum, include a disclaimer, and recommend professional confirmation. The goal is a credible starting point, not a final invoice.

**4. What happens if the vehicle model is identified incorrectly?**
Two safeguards. First, we show the prediction with a confidence score and let the user correct it before the price search runs, so a wrong guess doesn't silently propagate. Second, even if the model is off, the *damage detection and part identification* still work, because those don't depend on knowing the exact car — only the price step does, and that's the step the user can fix.

**5. How do you collect online part prices?**
Once we know the part, model, and year, the price-search module queries marketplaces and the web — ideally using an OEM or part number for the most precise match. It gathers compatible listings across conditions (new, used, aftermarket), filters out obvious outliers, and passes a clean set of prices to the cost engine.

**6. How do you handle prices that change constantly?**
We query *at the moment of assessment* rather than relying on a fixed price list, present a range instead of a single number, and timestamp the estimate so the user knows it's a snapshot. Historical price tracking is on our roadmap to make ranges more stable over time.

**7. Can the system detect hidden or internal damage?**
No — and we say so clearly. A photo only shows visible, external damage; it cannot reveal a bent frame or mechanical issues underneath. This is a stated limitation, and our report explicitly recommends a professional inspection for anything beyond surface damage. Future work could combine photos with sensor or repair-history data to approach this.

**8. Why is this better than just visiting a mechanic?**
It isn't a replacement — it's a *first step* that makes the mechanic visit better. In seconds, from your phone, you get an independent baseline: which part, what damage, and a rough cost range. You then walk into the shop informed, able to sanity-check the quote, instead of accepting one subjective number with no reference point.

**9. Does the system replace an insurance expert?**
No. It's a pre-assessment and triage tool. For insurers it can speed up and standardize the first pass on a claim and provide timestamped photo documentation, but the formal decision stays with the human expert. We're explicit that this is preliminary assistance, not an official determination.

**10. What datasets and evaluation metrics could you use?**
For damage, public datasets such as **CarDD** (car damage detection, with categories like dent, scratch, crack, and glass/lamp damage) and Roboflow car-damage sets; for vehicle recognition, datasets like **Stanford Cars** or **VMMRdb**; plus our own collected and annotated images. For evaluation we use **mAP** (mean Average Precision) at IoU thresholds — mAP@0.5 and mAP@0.5:0.95 — for detection, and precision, recall, F1, and a confusion matrix for damage-type classification.

**11. How do you measure model accuracy?**
We hold out a labeled test set the model never sees during training, then report detection metrics (mAP, IoU) and classification metrics (precision/recall/F1). We also inspect the confusion matrix to see *which* damage types get confused (for example, deep scratches vs. cracks) so we know where to improve rather than just quoting one headline number.

**12. How do you prevent incorrect or misleading results?**
Several layers: we show confidence scores on every prediction, set sensible confidence thresholds so low-certainty detections are flagged, let the user correct the vehicle identity, present cost as a range with a disclaimer, and keep a human in the loop throughout. The product is designed to *support* a decision, never to make a final one automatically.

**13. What makes this project innovative?**
The innovation is end-to-end *integration*, not a single new model. Most projects stop at detecting damage. We connect vehicle ID, damage detection, part recognition, damage classification, real online pricing, repair-vs-replace logic, and a generated report into one flow — turning a computer-vision demo into a decision-support tool that answers the question users actually have: *what do I do, and what will it cost?*

**14. Who are the main users?**
Individual drivers are the core users. Beyond that: insurance companies (claim pre-assessment), repair shops (estimating and documentation), rental and fleet companies (return/handover inspection), used-car marketplaces (objective condition reports), and inspection services (a faster, standardized first pass).

**15. How could this become a real commercial product?**
A few viable paths: a **B2B** model selling assessment/triage to insurers and fleets (per-assessment or subscription), a **freemium** consumer app where detailed reports or shop integration are premium, and **partnerships** with repair shops and marketplaces for referrals. The technical foundation — a modular pipeline — is built to scale into any of these without a rewrite.

**16. How do you handle different photo angles and poor lighting?**
We train on augmented data covering varied angles and lighting, validate image quality before running analysis (and prompt the user to retake if needed), and on the roadmap we plan multi-photo fusion so several angles of the same damage reinforce each other. We're realistic that very poor images reduce accuracy, which is why we surface confidence scores rather than hiding uncertainty.

**17. What about user privacy and the photos they upload?**
Photos can contain identifying details (plates, location, the vehicle itself), so we treat them as sensitive: process only what's needed for the assessment, be transparent about storage, and avoid retaining images longer than necessary. For an insurance/fleet deployment this would be formalized with proper data handling and consent.

**18. How does the system decide repair vs. replace?**
It uses the detected damage type and affected part. Shallow scratches and small dents are usually *repairable* (labor-heavy, no new part), while cracks, shattered glass, broken lamps, and structural deformation usually need *replacement*. We show both options with their cost ranges so the final choice stays with the user.
